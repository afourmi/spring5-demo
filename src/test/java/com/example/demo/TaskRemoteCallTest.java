...
public class TaskRemoteCallTest {
    ...
    public void testCreateTaskBlockingcall() {
        ...
        WebClient.RequestHeadersSpec<?> requestPreparation = taskEndPoint.post().uri("/taskBlocking").body(BodyInserters.fromPublisher(tasksFlux, Task.class));
    ...
    public void testCreateTaskNonBlockingcall() {
        Flux<Task> tasksFlux = Flux.just(
                new Task("New", Maps.newHashMap("firstName", "Robert")),
                new Task("New", Maps.newHashMap("firstName", "Alice")));
        WebClient.RequestHeadersSpec<?> requestPreparation = taskEndPoint.post().uri("/task").body(BodyInserters.fromPublisher(tasksFlux, Task.class));
